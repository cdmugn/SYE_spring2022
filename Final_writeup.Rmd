...
start_test_df <- turnout_data %>% 
  separate(Time_of_day, into = c("start", "end"), sep = "-") %>%
...
# hms(turnout_data$Start_time)

#as.duration(hms(`Start time`)) %>%

#turnout_data_time <- turnout_data %>% strptime(Start_time, format = "%H:%M", tz = "EST")

```

attempts that failed to convert the dates:
...
var_choice <- turnout_data %>% select(4,7,8,10)
...
ggplot(data = personality_df() %>% mutate(Personality= fct_reorder(Personality, Totalnegativesigns)), aes(x = .data[[input$varchoice]], y = Totalnegativesigns)) +
  geom_boxplot(color = "firebrick2", fill = "darkred") +
  theme_stata() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1, color = "white"), plot.title = element_text(color = "white"), plot.background = element_rect(fill = "tan4"), axis.text.y = element_text(color = "white"), axis.title = element_text(color = "white")) + 
