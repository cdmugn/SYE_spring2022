...
ui <- fluidPage(navlistPanel(tabPanel("Personality",
  sidebarLayout(sidebarPanel(
    selectizeInput("Personality",
                   label = "Choose a Personality Type", choices = levels(factor(turnout_data$Personality)), selected = "Aloof - Passive", multiple = TRUE)),
    mainPanel(plotOutput("boxplot1", height = 600),
              tableOutput("table1")),
    )
    ),
  tabPanel("Horse",
  sidebarLayout(sidebarPanel(
    selectizeInput("Horse",
                   label = "Choose a Horse", choices = levels(factor(turnout_data$Horse)), selected = "Juno", multiple = TRUE)),
    mainPanel(plotOutput("scatterplot", height = 600)),
      )
    )
  ))
...
server <- function(input, output, session) {
   ...
   personality_df <- reactive({turnout_data %>% filter(Personality == input$Personality)
   })
   meanpersonality <- reactive({negativepersonality %>% filter(Personality == input$Personality)
     })
   horse_df <- reactive({turnout_data %>% filter(Horse == input$Horse)
     })
